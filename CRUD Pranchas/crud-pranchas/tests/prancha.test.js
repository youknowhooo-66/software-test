...
describe('API de Pranchas - CRUD', () => {
  ...
  test('POST /pranchas - deve criar uma nova prancha', async () => {
    ...
    pranchaId = res.body.id; // salva para próximos testes
  });
  ...
  // UPDATE
  // =======================
  test('PUT /pranchas/:id - deve atualizar uma prancha', async () => {
    const prancha = await prisma.prancha.create({
      data: {
        nome: 'Prancha Atual',
        modelo: 'Modelo A',
    ...
    const res = await request(app)
      .put(`/pranchas/${prancha.id}`)
      .send({ preco: 1200, tipo: 'Shortboard' });

    expect(res.statusCode).toBe(200);
    expect(Number(res.body.preco)).toBe(1200);
    expect(res.body.tipo).toBe('Shortboard');
  });
  ...
  test('DELETE /pranchas/:id - deve deletar uma prancha', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body).toHaveProperty('mensagem', 'Prancha deletada com sucesso');
